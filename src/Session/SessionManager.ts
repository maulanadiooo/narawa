...
export class SessionManager {
    ...
    try {
      ...
      if (!decoded || !decoded.user) {
        throw new ErrorResponse(
          ...
          `Invalid WhatsApp number format: ${jid}`
        );
      }
  ...
  loadActiveSessions = async (): Promise<void> => {
    try {
        ...
        if (session.isActive && session.status === "connected") {
          ...
          try {
            ...
            printConsole.success(
              `Session ${session.sessionName} reloaded successfully`
            );
          } catch (error) {
            printConsole.error(
            ...
            await session.save();
            printConsole.info(
              `Session ${session.sessionName} marked as disconnected and deleted from database`
            );
      ...
      printConsole.error(
        `Failed to load active sessions: ${(error as Error).message}`
      );
    }
  };
  ...
  createSession = async (
    ...
    if (this.sessions.has(sessionName)) {
      throw new ErrorResponse(
        400,
        "SESSION_IS_ACTIVE",
        `Session '${sessionName}' already exists and active in memory`
      );
    }
    ...
    if (session) {
      if (session.isActive) {
        throw new ErrorResponse(
          400,
          "SESSION_IS_ACTIVE",
          `Session '${sessionName}' already exists and active`
        );
      }
  ...
  private handleConnectionUpdate = async (
    ...
    if (qr) {
      ...
      try {
        ...
        await (session as Session).save();
        printConsole.info(
          `QR code generated for session ${session.sessionName}`
        );
      } catch (error) {
        printConsole.error(
    ...
    if (connection === "close") {
      ...
      printConsole.success(
        `Session ${session.sessionName} connected successfully, new Login ${isNewLogin}`
      );
      ...
      if (
        session.isPairingCode &&
        session.phoneNumber &&
        session.pairingStatus === "pending"
      ) {
        const sessionData = this.sessions.get(session.sessionName);

        if (sessionData && !sessionData.socket.authState.creds.registered) {
          const code = await sessionData.socket.requestPairingCode(
            session.phoneNumber
          );
  ...
  private handleWaMessage = async (
    ...
    try {
      ...
      printConsole.warning(
        `Invalid sender JID: ${message.key.remoteJid}, skipping message`
      );
      return;
    }
      ...
      if (protoImageMessage) {
        ...
        if (buffer) {
          if (saveMediaTo && saveMediaTo?.toLowerCase() === "s3") {
            ...
            if (isUploaded) {
              printConsole.success(
                `Image ${message.key.id} uploaded to S3 ${url}`
              );
            } else {
              printConsole.error(
                `Image ${message.key.id} failed to upload to S3`
              );
            }
          } else {
            ...
            printConsole.success(
              `Image ${message.key.id} saved to local ${url}`
            );
          }
        }
        mediaType = "image";
        ...
        // download document
        const buffer = await this.createBuffer({
          mediaKey: protoDocumentMessage.mediaKey,
          directPath: protoDocumentMessage.directPath,
          url: protoDocumentMessage.url,
          mediaType: "document",
          message: message,
          sessionData: sessionData,
        });
        if (buffer) {
          if (saveMediaTo && saveMediaTo?.toLowerCase() === "s3") {
            ...
            if (isUploaded) {
              printConsole.success(
                `Document ${message.key.id} uploaded to S3 ${url}`
              );
            } else {
              printConsole.error(
                `Document ${message.key.id} failed to upload to S3`
              );
            }
          } else {
            ...
            url = `${websiteUrl}media/${keyPath}`;
            printConsole.success(
              `Document ${message.key.id} saved to local ${url}`
            );
          }
        }
        mediaType = "document";
        ...
        const buffer = await this.createBuffer({
          mediaKey: protoVideoMessage.mediaKey,
          directPath: protoVideoMessage.directPath,
          url: protoVideoMessage.url,
          mediaType: "video",
          message: message,
          sessionData: sessionData,
        });
        if (buffer) {
          if (saveMediaTo && saveMediaTo?.toLowerCase() === "s3") {
            ...
            if (isUploaded) {
              printConsole.success(
                `Video ${message.key.id} uploaded to S3 ${url}`
              );
            } else {
              printConsole.error(
                `Video ${message.key.id} failed to upload to S3`
              );
            }
          } else {
            ...
            url = `${websiteUrl}media/${keyPath}`;
            printConsole.success(
              `Video ${message.key.id} saved to local ${url}`
            );
          }
        }
        mediaType = "video";
        ...
        const buffer = await this.createBuffer({
          mediaKey: protoAudioMessage.mediaKey,
          directPath: protoAudioMessage.directPath,
          url: protoAudioMessage.url,
          mediaType: "audio",
          message: message,
          sessionData: sessionData,
        });
        if (buffer) {
          ...
          if (saveMediaTo && saveMediaTo?.toLowerCase() === "s3") {
            ...
            if (isUploaded) {
              printConsole.success(
                `Audio ${message.key.id} uploaded to S3 ${url}`
              );
            } else {
              printConsole.error(
                `Audio ${message.key.id} failed to upload to S3`
              );
            }
          } else {
            ...
            url = `${websiteUrl}media/${keyPath}`;
            printConsole.success(
              `Audio ${message.key.id} saved to local ${url}`
            );
          }
        }
        ...
        const buffer = await this.createBuffer({
          mediaKey: protoPtvMessage.mediaKey,
          directPath: protoPtvMessage.directPath,
          url: protoPtvMessage.url,
          mediaType: "ptv",
          message: message,
          sessionData: sessionData,
        });
        if (buffer) {
          if (saveMediaTo && saveMediaTo?.toLowerCase() === "s3") {
            ...
            if (isUploaded) {
              printConsole.success(
                `PTV ${message.key.id} uploaded to S3 ${url}`
              );
            } else {
              printConsole.error(
                `PTV ${message.key.id} failed to upload to S3`
              );
            }
          } else {
        ...
        if (buffer) {
          if (saveMediaTo && saveMediaTo?.toLowerCase() === "s3") {
            ...
            if (isUploaded) {
              printConsole.success(
                `Sticker ${message.key.id} uploaded to S3 ${url}`
              );
            } else {
              printConsole.error(
                `Sticker ${message.key.id} failed to upload to S3`
              );
            }
          } else {
            ...
            url = `${websiteUrl}media/${keyPath}`;
            printConsole.success(
              `Sticker ${message.key.id} saved to local ${url}`
            );
          }
        }
    ...
    if (
      ...
      try {
        ...
        if (!sessionExists) {
          printConsole.warning(
            `Session ${session.sessionName} not found in database, skipping message save`
          );
          return;
        }
  ...
  private createBuffer = async ({
    ...
    try {
      ...
      if (isRetry) {
        ...
        try {
          const newMessage = await sessionData.socket.updateMediaMessage(
            message
          );
          const result = this.getMediaData(newMessage, mediaType);
          if (!result) {
            printConsole.error(
              `Failed to get media data: ${(error as Error).message}`
            );
            return null;
          }
  ...
  private handleMessageUpdates = async (
    session: ISession,
    ...
    try {
      for (const update of updates) {
        ...
        if (status) {
          ...
          if (isRead) {
            printConsole.success(
              `Message ${update.key.id} marked as read for session ${session.sessionName}`
            );
          } else {
            printConsole.warning(
              `Message ${update.key.id} marked as ${ackString} for session ${session.sessionName}`
            );
          }
          const sqlUpdateMessageReadu = `UPDATE messages SET is_read = ?, ack = ?, ack_string = ? WHERE message_id = ?`;
          await db.query(sqlUpdateMessageReadu, [
            isRead,
            status,
            ackString,
            update.key.id,
  ...
  private handleMessagingHistorySet = async (
    session: ISession,
    m: {
      chats: Chat[];
      contacts: Contact[];
      ...
      const contactPromise = (async () => {
        for (const contact of contacts) {
          ...
          if (phoneNumber.includes("@lid")) {
            ...
            if (sessionData) {
              const PNFromLID =
                await sessionData.socket.signalRepository.lidMapping.getPNForLID(
                  phoneNumber
                );
  ...
  > => {
    ...
    try {
      ...
      printConsole.error(
        `Failed to get sessions from database: ${(error as Error).message}`
      );
    }
  ...
  async deleteSession(sessionName: string): Promise<void> {
    ...
    if (sessionData) {
      try {
        ...
        printConsole.warning(
          `Error during socket logout: ${(error as Error).message}`
        );
      }
  ...
  async sendMessage(
    ...
    if (!sessionData) {
      ...
      throw new ErrorResponse(
        400,
        "SESSION_NOT_FOUND",
        `Session ${sessionName} not found or not active`
      );
      // const session = await this.sessionModel.findBySessionName(sessionName);
    ...
    try {
      ...
      switch (type) {
        case "text":
          result = await sessionData.socket.sendMessage(
            normalizedTo,
        ...
        case "image":
          ...
          if (!imageData.url && !imageData.buffer) {
            throw new ErrorResponse(
              400,
              "IMAGE_DATA_REQUIRED",
              `Image should as string url or File`
            );
          }
        ...
        case "document":
          ...
          if (!docData.url && !docData.buffer) {
            throw new ErrorResponse(
              ...
              `Document should as string url or File`
            );
          }
          result = await sessionData.socket.sendMessage(
            normalizedTo,
  ...
  sendRead = async (session: ISession, to: string, messageIds?: string[]) => {
    ...
    if (sessionData) {
      if (messageIds && messageIds.length > 0) {
        ...
        // check databasse
        const sql =
          "SELECT message_id, id FROM messages WHERE session_id = ? AND from_me = ? AND is_read = ? AND event = ?";
        const dataMessages = await db.query(sql, [
          session.id,
          0,
          0,
          "message.received",
        ...
        if (dataToRead.length > 0) {
          ...
          if (ids.length) {
            ...
            await db.query(
              `UPDATE messages SET is_read = ? WHERE id IN (${placeholders})`,
              [1, ...ids]
            );
          }
        } else {
          printConsole.warning(
            `No messages to read for session ${session.sessionName}`
          );
        }
      }
    }
